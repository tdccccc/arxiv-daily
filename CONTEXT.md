...
| **Library similarity** | Retrieval of the library papers most similar to a given paper or description, over the local full-text knowledge base. The product surface lists ranked papers and can open the local PDF; passage evidence may exist inside the index but is not shown until it has a measured quality bar. Distinct from lexical daily-report similarity, which searches papers that already appeared in daily reports. |
