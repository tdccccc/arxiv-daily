...
| **Library processing consent** | The researcher's informed authorization for eligible files from a chosen library to be processed through the named model endpoint at the disclosed depth. The product asks again when the endpoint changes or processing expands from metadata and abstracts to full text. Unrelated files are not implicitly included, and the researcher may revoke authorization. |
