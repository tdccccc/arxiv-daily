...
把图书馆引导式发现从"每日展示"闭环成"保存待读 → 按方向回顾 → 阅读反馈回流"，先修三处 P0 质量/安全缺陷（方向选题偏置、搜索模型校验、索引异常隔离），随后补文献库分析（先规模加固再洞察功能）。这是 2026-08-03-personal-library-guided-discovery（ADR 0004 步骤 1–6）的延续。
...
- [ ] 用户可将日报中发现的论文保存为待读候选，并按方向回顾做出阅读决策（精读/略读/弃）。
- [ ] 阅读反馈（dispositions）落库并回流影响后续发现。
