...
- [ ] 未选文件夹时，文献库这一行只提示选文件夹。
- [ ] 已选文件夹且为本地嵌入时，主按钮是建索引；不要求先授权。
- [ ] 已选文件夹且为远程嵌入时，主按钮先是授权；授权后变成建索引。
- [ ] 建索引可从设置页启动，进度和结果有通知；不必先打开命令面板。
- [ ] 授权文案区分本地（不离开本机）和远程（全文会发到嵌入接口）。
...
1. P1 — 设置页文献库一行给出当前下一步（选文件夹 / 授权 / 建索引） — status: active
2. P2 — 建索引过程与完成后的下一步（去搜索）可感知 — status: pending
