...
- [ ] Hosted 客户端接受精确 `{ "ok": true }` 及两种严格白名单的旧 Worker 成功体。
- [ ] 旧响应中的 provider ID 仅用于契约识别，验证后立即丢弃，不进入日志、状态或公开结果。
- [ ] 其他额外字段、非法类型、空或超长 ID 继续 fail closed，并阻断 automatic 自动重发。
- [ ] Core 定向测试、完整门禁和技术报告同步通过。
...
1. P1 — 严格识别并丢弃旧 Worker 成功响应 — status: active
