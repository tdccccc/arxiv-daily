...
- Schema: document schema version `1`, exact top-level and entry keys, date binding, ISO timestamps, and independently decoded entries keyed by canonical `paperKey` (`arxiv:<id>`). The map key, entry `paperKey`, fingerprint paper identity, and bare result summary ID must agree. `DailyPaperResult` structured values reuse generation's trim/scientific-math validator; fallback attempts are integers from `1` through `DAILY_PAPER_SUMMARY_MAX_ATTEMPTS`, and every fallback `originalAbstract` must exactly equal the fingerprint input's trusted original abstract. Persisted prompt/result contract versions must equal the current versions. Invalid entries are skipped with an observable warning while valid siblings survive; tolerant lookup falls back to a valid backup and otherwise returns a miss.
- Compatibility: SHA-256 over a fixed-order version-2 effective-input object containing canonical paper identity, title/authors, trusted fallback abstract, prompt-trimmed abstract/conclusion and nonblank full sections, normalized summary language, provider, an `endpointDigest`, model, provider-specific generation mode, and prompt/result contract versions. The endpoint digest hashes the exact effective `LlmClient` chat request URL, including request-significant base URL userinfo, host, port, path, query, and fragment semantics; no plaintext endpoint host/path/query/userinfo is persisted. Non-thinking mode records effective temperature only; thinking mode excludes ignored temperature and records either Anthropic's effective budget or the effective reasoning effort. API keys and redundant raw inputs are excluded.
...
- Mutation: same normalized document paths share one process-local queue across store instances. Mutation loading is strict: missing primary and backup starts empty, and a missing or structurally corrupt primary may continue from a valid backup. An I/O-unreadable primary always rejects without consulting a potentially stale backup; primary/backup corruption with no valid copy also rejects without overwriting state. The core store exclusively owns recoverable `.tmp`/`.bak` replacement instead of composing host `writeTextAtomic` suffix behavior, and retains the last valid primary as backup across successful writes. A failed mutation restores the previous valid primary and does not poison later queued mutations.
