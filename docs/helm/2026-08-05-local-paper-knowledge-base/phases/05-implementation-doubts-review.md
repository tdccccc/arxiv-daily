...
- [x] T2 存疑项 3（discoveryCues 截断）：定位 "new" 候选 `discoveryCues` 填充路径（reason 截断单条）→ 定修法（填充源 vs decoder 放宽，评估其它路径影响）→ 修复 + 测试
  - 结论：截断发生在 plugin `attachNewSuggestionToProposal`（core 的 `buildNewDirectionDraft` 返回空 cues，插件用 description 截断单条）；decoder 要求 ≥1 且严格升序唯一（≤12 条、≤200 字符）。修法＝填充源改进：代表论文标题（catalog 可得）去重 + code-unit 排序后作 cues，空回退 reason；core `derivedCues` 保持（merge/split 方向无标题可用，注释已声明 draft placeholder）。审核 UI 本就允许编辑 cues。+1 测试断言（标题派生 + durable decode 通过）；technical-report handoff no-impact。
- [x] T3 存疑项 2（operation kind 复用）：核对增量更新与全量生成共用 `personal-library-direction-generation` 的授权门/撤销范围/取消语义 → 定论（保持复用 or 新 kind）→ 记录与 ADR 0007 拆分授权门的衔接
  - **结论：保持复用（有意为之，非疏漏）**。核对证据：①同一授权门域（方向生成工作，`getLibraryConnectionStatus().kind !== "authorized"` 双侧一致）；②共享撤销取消范围——`cancelPersonalLibraryDirectionGeneration` 在授权撤销（main.ts:531/536）与端点变更（:558）时取消该 kind 全部运行（全量+增量）；③同 kind+同 scopeFingerprint key 提供互斥——全量与增量都写同一 profile/proposal store，并发互斥是必需的。**新增 kind 有害**：会破坏互斥（需另行实现跨 kind 排除）、扩大封闭 union、撤销清单双处维护。**与 ADR 0007 衔接**：授权门拆分（placement 免许可、LLM diff 要许可）在 **plugin 门检查层**实现，不经 operation kind——运行仍注册共享 kind；P6 实现提示：把 run-entry 的授权检查（现 main.ts:997）下移到 LLM diff 阶段，`assertIncrementalUpdateCurrent` 的 authorizationFingerprint 检查保留给 LLM 阶段，无许可时 placement 照跑并记"待授权"（替代现 run-entry throw）；placement-only 运行被撤销取消无副作用（本地嵌入，下次索引重跑）。technical-report 已记载复用语义（line 290），无需改动。
