...
- [ ] Define and test the host-neutral catalog schema, strict decoder, scan summary, paper/file outcome semantics, scope/identification fingerprint, and storage path under the existing index root.
- [ ] Implement a durable catalog store with valid-backup recovery, fail-closed mutation on unreadable current/backup data, path-scoped serialization, atomic whole-document writes, and deterministic semantic revisioning.
