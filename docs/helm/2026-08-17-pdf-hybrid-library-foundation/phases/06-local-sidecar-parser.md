...
- [ ] 实现并验收 host-neutral per-parse selector 与可选 client/parser adapter：未启用时不调用 endpoint，probe/transport/schema failure 以 PDF.js identity 回退，cancel 不降级，sidecar 不影响旧 `DocumentParser` 或 default local path。
