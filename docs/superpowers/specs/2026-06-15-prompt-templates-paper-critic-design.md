...
### 新增领域专家人设（persona，2026-06-15 确认）

开场白由「你是一个专业的研究助手」改为按论文 topic 注入的领域专家：

> 你是一位资深研究者，专长于「{{topicName}}」相关领域。请根据提供的论文各章节内容……

新增模板变量 `{{topicName}}`；`summarizePaperDetail` 由 `paper.category` 经 `arxivSettings.topics` 反查显示名（`topic?.name || paper.category`）传入。persona **仅用于详细总结**——筛选与日报是多 topic / 分类任务，领域人设不适用，保持原样。
